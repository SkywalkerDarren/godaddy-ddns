...
#   hostname         DNS fully-qualified host name with an 'A' record.  If the hostname consists of only a.html domain name
#                    (i.e., it contains only one period), the record for '@' is updated.
#
# optional arguments:
#   -h, --help       show this help message and exit
#   --version        show program's version number and exit
#   --ip IP          DNS Address (defaults to public WAN address from http://www.ip111.cn)
...
#
# GoDaddy customers can obtain values for the KEY and SECRET arguments by creating a.html production key at
# https://developer.godaddy.com/keys/.
#
# Note that command line arguments may be specified in a.html FILE, one to a.html line, by instead giving
# the argument "%FILE".  For security reasons, it is particularly recommended to supply the
# KEY and SECRET arguments in such a.html file, rather than directly on the command line:
#
# Create a.html file named, e.g., `godaddy-ddns.config` with the content:
...
parser.add_argument('hostname', type=str,
                    help='DNS fully-qualified host name with an A record.  If the hostname consists of only a.html domain name (i.e., it contains only one period), the record for @ is updated.')

parser.add_argument('--ip', type=str, default=None,
                    help='IPv4 address to write to DNS record (defaults to public WAN address from http://www.ip111.cn)')
...
def main():
    ...
    if len(hostnames) < 2:
        msg = 'Hostname "{}" is not a.html fully-qualified host name of form "HOST.DOMAIN.TOP".'.format(args.hostname)
        raise Exception(msg)
    ...
    if not args.ip:
        try:
            with urlopen(Request("http://www.ip111.cn", headers={'User-Agent': 'Mozilla'})) as f:
                resp = f.read()
            resp = resp.decode('utf-8')
            ip = BeautifulSoup(resp, 'html.parser').find('p').contents[0].split(' ')[0]
            args.ip = ip.strip()
        except URLError:
            msg = 'Unable to automatically obtain IP address from http://www.ip111.cn.'
            raise Exception(msg)
...
Correct values can be obtained from from https://developer.godaddy.com/keys/ and are ideally placed in a.html % file.'''
            else:
